...
public class EquipmentService {
    ...
    public void delete(String code) {
        ...
        //判断是否存在
        Equipment findOne = equipmentRepository.findOne(code);
        if (findOne == null) {
            throw new MesException(EnumException.DELETE_FAILED_NOT_EXIST);
        }

        Equipment equipment = new Equipment();
        equipment.setCode(code);
        if(eqRepairApplicationRepository.findFirstByEquipment(equipment) != null){
            throw new MesException(EnumException.DELETE_FAILED_REPAIR_REF);
        }
