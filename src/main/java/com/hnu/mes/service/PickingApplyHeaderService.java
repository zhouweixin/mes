...
public class PickingApplyHeaderService {
	...
	public PickingApplyHeader update(PickingApplyHeader pickingApplyHeader) {
		...
		// 判断是否存在
		PickingApplyHeader one = pickingApplyHeaderRepository.findOne(pickingApplyHeader.getCode());
		if (one == null) {
		...
		if (one.getAuditStatus() != GlobalUtil.PickingApplyHeaderStatus.NOT_SUBMIT) {
			throw new MesException(EnumException.UPDATE_FAILED_SUBMIT_NOT_ALLOW);
		}

		pickingApplyHeaderRepository.delete(pickingApplyHeader.getCode());

		return save(pickingApplyHeader);
	}
