...
var equipment_manage = {
	...
	do_once: function() {
		$.get(home.urls.department.getAll(), function(result) {
            equipment_manage.department_result = result.data;
		})

		$.get(home.urls.productline.getAll(), function(result) {
            equipment_manage.productline_result = result.data;
		})

		$.get(home.urls.user.getAll(), function(result) {
            equipment_manage.user_result = result.data;
		})
	},
	init: function() {
	...
	funcs: {
		...
		renderHandler: function($tbody, equipments) {
			$tbody.empty() //清空表格
